...
class ReferenceIntegrityTests(unittest.TestCase):
    def test_family_registry_is_complete(self) -> None:
        ...
        self.assertEqual(sum(row["expected_count"] for row in families), 18_163)
        self.assertEqual(len({row["id"] for row in families}), 42)
        self.assertEqual(len({row["json"] for row in families}), 42)

    def test_structural_audit_passes(self) -> None:
        errors, summary = reference_audit.structural_audit()
        self.assertEqual(errors, [])
        self.assertEqual(summary["families"], 42)
        self.assertEqual(summary["rows"], 18_163)
