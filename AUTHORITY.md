...
| `whfb_wargame_harvest.py` | cited WHFB *wargame* unit profiles from BORN-DIGITAL 8th-ed army-book PDFs (PyMuPDF words-mode geometric grid, glyph-cluster name reconstruction, live header read); a corruption GATE (alpha-token junk-fraction) routes broken-CMap PDFs to NO COVERAGE; fan-made books labeled `official:false` and never harvested; a SEPARATE `system: WHFB` index |
| `wh40k_wargame_harvest.py` | cited WH40K *wargame* unit profiles from BORN-DIGITAL codex PDFs (PyMuPDF words-mode geometric grid reconstruction); auto-detects digital vs scanned (45 scanned print NO COVERAGE); generic header read (infantry WS…Sv + vehicle Front/Side/Rear); a SEPARATE `system: WH40K` index; 2nd-ed image-only labels captured positionally + flagged soft, never fabricated |
