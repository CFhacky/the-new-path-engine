...
| `mystery_harvest.py` | cited Tome of Magic shadow-magic descriptions; heading/category/Level-School detector captures 9 fundamentals and 60 path mysteries with printed fields only and exact full-description spans on PDF pp.142–154 (native D&D 3.5e, its own index); shared chapter defaults are not imputed into rows; truename utterances remain separate; missing sources print NO COVERAGE |
