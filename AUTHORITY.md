...
| `item_harvest.py` | cited magic-item extractions (Magic Item Compendium; DMG v3.5; Arms & Equipment Guide 3.0); all 1,454 rows carry exact per-source OCR paths and validated full-description spans, with three source-verified damaged-heading aliases; the additive DMG path joins 34 table-first rod/staff rows to Tables 7-19/7-25 and their descriptions while locking all 1,420 prior rows; A&EG rejects CHAPTER running headers and locks all 362 accepted rows; missing sources print NO COVERAGE; conversions require both 3.5e and GURPS halves |
| `power_harvest.py` | cited psionics extractions (Expanded Psionics Handbook + Complete Psionic); all 409 mechanical rows retain exact source paths, 406 carry validated non-overlapping full-description spans, and three column-interleaved XPH rows remain explicit NO COVERAGE; Complete Psionic running headers are removed only as page furniture; missing sources print NO COVERAGE; conversions require both 3.5e and GURPS halves |
...
| `teatd_realm_harvest.py` | *The End and the Death* Vols I–III (Notion Horus Heresy Source Library; sweep 2026-09-21): 55 realm rows, every attestation located in the volume text at chapter:verse (`corpus-exact`) when `--corpus` is present, NO COVERAGE otherwise; `facts` SOURCE-VERIFIED, `myth_key` INFERRED and labelled so; a SEPARATE `system: WH40K Novel` index; never canon |
| `realm_map.py` | `reference/teatd_realm_index.json` + empyrean-siege-gm TEATD band rule (map contradiction and false continuity are hazards); rolls once with `secrets`, renders deterministically from `*.realm.state.json`; refuses `condition` rows; archetype tables are APPROVED PREPARATION pending ratification. Campaign realms enter only through `--realm-file` from the owning project (CAMPAIGN-ORIGINAL), never as rows here. `populate` rolls denizens as citations into the engine's own bestiary rows (name, book, page, wounds or CR) and restates no stat; `export-sectors` emits the area list for realm_population.py (vault) to populate, since that project owns its 266 and its D2 corpus; A built place (`fixed_areas`, `fixed_edges`, `anchored`, `grammar`, `growth`) is READ from its definition file rather than thrown for, and `grow` adds one interval of new work without re-rolling the standing areas, edges or law |
| `notion_novel_bind.py` | corpus tooling: strips the Notion fetch wrapper from a long-form page dump and binds EPUB 3 with the standard library; the text passes through untouched; outputs live under `corpus/teatd/` in this repository |
| `verse_address.py` | a dumb, stable grid over a book in the manner of chapter-and-verse: unit addresses, EPUB anchors, line spans, incipits, edition SHA-256; corpus metadata only, never body text, never canon; not a families.json family |
| `canon_table.py` | `reference/canon/events.json` (authored registry: witnesses by pinned address, narrator, narration mode, stated synchronism, short quote) + the address maps; renders `docs/canon/`; `--check` fails on an unresolved address, a drifted edition or a quote missing from its unit; narrator and mode are INFERRED, quotes SOURCE-VERIFIED when corpus-exact, synchronisms only what the verse states; never a calendar, never a harmony |
| `gurps_gear_harvest.py` | cited GURPS Low-Tech weapon + armor tables plus the bounded 18-row GURPS Ultra-Tech Concealable Ballistic Armor Table; layout-specific column-dump parsers with exact, non-overlapping source-row spans (native GURPS 4e gear, its own index); missing sources print NO COVERAGE |
| `gurps_trait_harvest.py` | cited GURPS Basic Set Trait Lists roster plus exact, non-overlapping description/inline-definition spans on B18–B165 (four source-shared pairs), additively extended with the five GURPS Powers New Advantages absent from the Basic roster (P90–P98; Neutralize remains the existing Basic row); native GURPS 4e, its own index; missing or unclean sources print NO COVERAGE |
...
| `utterance_harvest.py` | cited Tome of Magic truename utterance rosters + Level-anchored detail blocks (born-digital text); parser reconciles 65 entries across the Evolving Mind, Crafted Tool, and Perfected Map lexicons, preserves printed normal/reverse summaries and true full-description spans on pp.235–253, and records the book's missing distinct Perfected Map base DC as NO COVERAGE (native D&D 3.5e, its own index); missing sources print NO COVERAGE |
...
| `whfb_wargame_harvest.py` | cited WHFB *wargame* unit profiles **plus book-verbatim SPECIAL RULES blocks** from six BORN-DIGITAL 8th-ed army-book PDFs (PyMuPDF words-mode geometric grid + subject/page-reference rule attachment) plus one bounded, page-image-verified High Elves batch (PDF p.91 only); 228/302 profiles carry rules with their own page citations and 74 ambiguous rows are named NO COVERAGE; all other High Elves pages remain NO COVERAGE; a corruption GATE routes broken-CMap PDFs to NO COVERAGE; fan-made books are never harvested; a SEPARATE `system: WHFB` index |
...
| `prestige_class_harvest.py` | D&D 3.x prestige classes: the compiled Dragon Magazine Prestige Class Compendium PDF/page remains visual authority for canonical mechanics, while every one of the 145 rows now carries an explicit Dragon issue and manifest key for OCR-first description recovery; 143 are routed to issue OCR and Knight of the Chase / Master of the Secret Sound remain explicit OCR gaps in their attributed #297 source; the resolver follows each manifest `ocr_output_md`, derives `skipped_has_text` fallback locations from manifest peers, and forbids the sparse compiled Markdown export; 23 recovered rows retain validated exact spans and separate compiled-PDF provenance |
