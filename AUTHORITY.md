...
| `realm_map.py` | `reference/teatd_realm_index.json` + empyrean-siege-gm TEATD band rule (map contradiction and false continuity are hazards); rolls once with `secrets`, renders deterministically from `*.realm.state.json`; refuses `condition` rows; archetype tables are APPROVED PREPARATION pending ratification; no denizens, stats or items |
