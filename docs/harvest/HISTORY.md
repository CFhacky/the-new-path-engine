...
| `reference/gurps_gear_index.{md,json}` | `scripts/gurps_gear_harvest.py` | GURPS Low-Tech Melee Weapon + Armor Tables; GURPS Ultra-Tech Concealable Ballistic Armor Table | 204 gear (153 Low-Tech weapons + 33 Low-Tech armor pieces + 18 Ultra-Tech TL9–11 concealable armor pieces; all carry exact non-overlapping full-row spans) | `python scripts/gurps_gear_harvest.py --selftest` |
...
- **2026-08-30** — Extended the native GURPS 4e gear family with the bounded
  *Ultra-Tech* Concealable Ballistic Armor Table on printed p.172 / PDF p.173.
  The new layout-specific detector accepts exactly 18 TL9–11 rows and preserves
  each printed TL, name, protected location, split DR, cost, weight, and LC in
  an exact seven-cell source span. All 186 Low-Tech rows remain byte-identical;
  the family rises **186 → 204**, total accepted entries **18,278 → 18,296**,
